...
export function ContactExportDialog({
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/contacts/export?teamId=${teamId}`,
        {
          method: "POST",
          headers: {
  ...
  }, [contactIds, selectedFields, selectedAttributes, teamId, toast, onOpenChange]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose the fields and attributes to include in the CSV export for the selected {contactIds.length} contact{contactIds.length === 1 ? "" : "s"}.
          </DialogDescription>
        </DialogHeader>
